...
//  GassExpress
//
//  Created by Samuel Menendez on 10/15/23.
//
import SwiftUI
    ...
    var body: some View {
        VStack {
            ...
            VStack(alignment: .leading) {
                ...
                // Agrega la navegación a "ActualizarC"
                         NavigationLink(destination: ActualizarC(), isActive: $isShowingActualizarC) {
                             EmptyView()
                         }

                         // Botón para "Olvidaste tu contraseña?" que navega a ActualizarC
                         Button(action: {
                             isShowingActualizarC.toggle()
                         }) {
